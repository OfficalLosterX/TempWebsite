...
const OUT_FILE = path.join(ROOT, 'media.json'); // <-- write to repo root
...
function parseKeyValue(text) {
  ...
  text.split(/\r?\n/).forEach(line => {
    ...
    if (/^tags?$/i.test(key)) value = value.split(',').map(s => s.trim()).filter(Boolean);
    out[key] = value;
  });
  return out;
}
...
(async () => {
  try {
    ...
    for (const f of files) {
      ...
      if (!raw) continue;
      let parsed = null;
      try { parsed = JSON.parse(raw); } catch (e) { parsed = parseKeyValue(raw); }
